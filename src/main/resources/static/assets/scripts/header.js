...
function showFlyout(triggerSelector, flyoutSelector, xOffset = 0, yOffset = 0) {
    ...
    // ✅ 이미 열린 상태이고, 같은 트리거나 같은 팝업이면 닫기 (토글)
    if (visibleFlyoutSelector === flyoutSelector && visibleTriggerSelector === triggerSelector) {
        hideAllFlyouts();
        return;
    }
...
function updateFlyoutPosition() {
    ...
    showFlyout(visibleTriggerSelector, visibleFlyoutSelector, offsetX, offsetY);    // 현재 위치, 정보를 이용해 다시 계산해 맞춤!
}
